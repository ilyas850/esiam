...
<?php $__env->startSection('content'); ?>
    ...
    <section class="content">
        <div class="row kuisioner-info">
            <div class="col-sm-6"><div class="info-box"><span class="info-box-icon bg-aqua"><i class="fa fa-university"></i></span><div class="info-box-content"><span class="info-box-text">Layanan yang Dinilai</span><span class="info-box-number">Biro Administrasi Akademik dan Kemahasiswaan</span></div></div></div>
            ...
            <div class="col-sm-3"><div class="info-box"><span class="info-box-icon bg-green"><i class="fa fa-users"></i></span><div class="info-box-content"><span class="info-box-text">Kelas</span><span class="info-box-number"><?php echo e($kelas); ?></span></div></div></div>
        </div>

        <div class="callout callout-info"><h4><i class="fa fa-info-circle"></i> Petunjuk Pengisian</h4><p>Pilih satu nilai untuk setiap pernyataan layanan BAAK. Semua pertanyaan wajib diisi sebelum kuisioner dapat disimpan.</p><span class="label label-danger">1 Tidak Baik</span> <span class="label label-warning">2 Kurang Baik</span> <span class="label label-info">3 Baik</span> <span class="label label-success">4 Sangat Baik</span></div>

        <?php if($errors->any()): ?>
            <div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> Mohon lengkapi seluruh pertanyaan sebelum menyimpan.</div>
        <?php endif; ?>

        <form action="<?php echo e(url('save_kuisioner_baak')); ?>" method="post" id="kuisioner-form">
            ...
            <div class="box box-info">
                <div class="box-header with-border"><h3 class="box-title">Pernyataan Penilaian Layanan BAAK</h3><div class="box-tools pull-right"><span class="label label-info"><?php echo e($questionnaireQuestionCount); ?> pertanyaan</span></div></div>
                ...
                <div class="table-responsive"><table class="table table-hover kuisioner-table"><thead><tr><th width="6%" class="text-center">No</th><th width="23%">Aspek</th><th>Komponen</th><th width="45%" class="text-center">Penilaian</th></tr></thead><tbody>
                    <?php $no = 1; ?>
                    <?php $__currentLoopData = $data; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php $selectedValue = old('nilai.' . $item->id_kuisioner); ?>
                        <tr class="kuisioner-question-row">
                            <td class="text-center kuisioner-question-number" data-label="Pertanyaan"><?php echo e($no++); ?></td><td data-label="Aspek"><?php echo e($item->aspek_kuisioner); ?></td><td data-label="Komponen"><?php echo e($item->komponen_kuisioner); ?></td>
                            <td class="text-center" data-label="Penilaian">
                                <label class="btn btn-default btn-xs kuisioner-rating-option <?php echo e($selectedValue == $item->id_kuisioner . ',1' ? 'is-selected' : ''); ?>"><input class="kuisioner-answer" type="radio" name="nilai[<?php echo e($item->id_kuisioner); ?>]" value="<?php echo e($item->id_kuisioner); ?>,1" required <?php echo e($selectedValue == $item->id_kuisioner . ',1' ? 'checked' : ''); ?>> 1 Tidak Baik</label>
                                <label class="btn btn-default btn-xs kuisioner-rating-option <?php echo e($selectedValue == $item->id_kuisioner . ',2' ? 'is-selected' : ''); ?>"><input class="kuisioner-answer" type="radio" name="nilai[<?php echo e($item->id_kuisioner); ?>]" value="<?php echo e($item->id_kuisioner); ?>,2" <?php echo e($selectedValue == $item->id_kuisioner . ',2' ? 'checked' : ''); ?>> 2 Kurang Baik</label>
                                <label class="btn btn-default btn-xs kuisioner-rating-option <?php echo e($selectedValue == $item->id_kuisioner . ',3' ? 'is-selected' : ''); ?>"><input class="kuisioner-answer" type="radio" name="nilai[<?php echo e($item->id_kuisioner); ?>]" value="<?php echo e($item->id_kuisioner); ?>,3" <?php echo e($selectedValue == $item->id_kuisioner . ',3' ? 'checked' : ''); ?>> 3 Baik</label>
                                <label class="btn btn-default btn-xs kuisioner-rating-option <?php echo e($selectedValue == $item->id_kuisioner . ',4' ? 'is-selected' : ''); ?>"><input class="kuisioner-answer" type="radio" name="nilai[<?php echo e($item->id_kuisioner); ?>]" value="<?php echo e($item->id_kuisioner); ?>,4" <?php echo e($selectedValue == $item->id_kuisioner . ',4' ? 'checked' : ''); ?>> 4 Sangat Baik</label>
                            </td>
                        </tr>
    ...
    <script>
        (function () {
            ...
            function updateProgress() { var answered = 0; Array.prototype.forEach.call(document.querySelectorAll('.kuisioner-question-row'), function (row) { var selectedAnswer = row.querySelector('.kuisioner-answer:checked'); if (selectedAnswer) { answered++; } row.classList.toggle('is-answered', selectedAnswer !== null); Array.prototype.forEach.call(row.querySelectorAll('.kuisioner-rating-option'), function (option) { option.classList.toggle('is-selected', option.querySelector('.kuisioner-answer:checked') !== null); }); }); var percentage = total ? Math.round((answered / total) * 100) : 0; progressBar.style.width = percentage + '%'; progressText.textContent = answered + ' dari ' + total + ' pertanyaan terisi'; answerCount.textContent = answered; saveButton.disabled = answered !== total; }
            Array.prototype.forEach.call(answers, function (answer) { answer.addEventListener('change', updateProgress); }); updateProgress();
        })();
    </script>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/resources/views/mhs/kuisioner/kuisioner_baak.blade.php ENDPATH**/ ?>