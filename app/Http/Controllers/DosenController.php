...
class DosenController extends Controller
{
    ...
    function persentase_absensi_mhs($id)
    {
        ...
        return view('dosen/mhs/persentase_absen', compact('data', 'mk'));
    }
    ...
    public function unduh_pdf_nilai($id)
    {
        ...
        $pdf = PDF::loadView('dosen/unduh_nilai_pdf', ['d' => $d, 'm' => $m, 'y' => $y, 'data' => $key, 'tb' => $mhs_kelas, 'nilai' => $setting_nilai])->setPaper('legal', 'landscape');
    ...
    public function input_partisipatif_dsn($id)
    {
        $kelas_gabungan = Helper::getMahasiswaDosen($id);
        return view('dosen/input_partisipatif_dsn', ['kuri' => $id, 'ck' => $kelas_gabungan, 'id' => $id]);
    }

    public function save_nilai_partisipatif_dsn(Request $request)
    {
        ...
        return redirect('/cekmhs_dsn/' . $request->id_kurperiode);
    }

    public function input_proyek_dsn($id)
    {
        $kelas_gabungan = Helper::getMahasiswaDosen($id);
        return view('dosen/input_proyek_dsn', ['kuri' => $id, 'ck' => $kelas_gabungan, 'id' => $id]);
    }

    public function save_nilai_proyek_dsn(Request $request)
    {
    ...
    public function input_tugas_dsn($id)
    {
        $kelas_gabungan = Helper::getMahasiswaDosen($id);
        return view('dosen/input_tugas_dsn', ['kuri' => $id, 'ck' => $kelas_gabungan, 'id' => $id]);
    }

    public function save_nilai_tugas_dsn(Request $request)
    {
    ...
    public function input_kuis_dsn($id)
    {
        $kelas_gabungan = Helper::getMahasiswaDosen($id);
        return view('dosen/input_kuis_dsn', ['kuri' => $id, 'ck' => $kelas_gabungan, 'id' => $id]);
    }

    public function save_nilai_kuis_dsn(Request $request)
    {
    ...
    public function generate_nilai_akhir_dsn_dlm(Request $request)
    {
        ...
        if (!$set_nilai) {
            ...
            return redirect('/cekmhs_dsn/' . $idkur);
        }
    ...
    public function input_bap($id)
    {
        ...
        return view('dosen/form_bap', compact('id', 'jam', 'nilai_pertemuan', 'rps', 'cek_mk'));
    }
    ...
    public function save_bap(Request $request)
    {
        ...
        $cek_bap = Bap::where('id_kurperiode', $id_kurperiode)
        ...
        foreach ($kelas_gabungan as $index => $kurperiode) {
            ...
            // Create BAP record
            $bap = new Bap([
                'id_kurperiode' => $id_kur,
                'id_dosen' => $id_dosen,
                'pertemuan' => $request->pertemuan,
                'tanggal' => $request->tanggal,
                'jam_mulai' => $request->jam_mulai,
                'jam_selsai' => $request->jam_selsai,
                'jenis_kuliah' => $request->jenis_kuliah,
                'id_tipekuliah' => $request->id_tipekuliah,
            ...
            // Save kuliah transaction
            $this->saveKuliahTransaction($request, $id_kur, $id_dosen, $bap->id_bap);
        }
    ...
    // Save kuliah transaction
    private function saveKuliahTransaction($request, $id_kur, $id_dosen, $id_bap)
    {
        Kuliah_transaction::create([
            'id_kurperiode' => $id_kur,
            'id_dosen' => $id_dosen,
            'id_tipekuliah' => $request->id_tipekuliah,
            'tanggal' => $request->tanggal,
            'akt_jam_mulai' => $request->jam_mulai,
            'akt_jam_selesai' => $request->jam_selsai,
            'id_bap' => $id_bap,
        ]);
    }
    ...
    public function cari_mhs_lintas_kelas($id_bap)
    {
        ...
        foreach ($students as $mhs) {
            ...
            $status_asal = 'Belum Ada BAP';
            if ($originBap) {
                $abs = Absensi_mahasiswa::where('id_bap', $originBap->id_bap)
                    ->where('id_studentrecord', $mhs->id_studentrecord)
                    ->first();
                if ($abs) {
                    $status_asal = ($abs->absensi === 'ABSEN' ? 'Hadir' : $abs->absensi);
                } else {
                    $status_asal = 'Belum Diabsen';
                }
            }
            $mhs->status_asal = $status_asal;
        }
    ...
    public function entri_absen($id)
    {
        ...
        // 3. Deteksi apakah ada mahasiswa reguler yang sudah hadir di kelas lain untuk pertemuan ini
        if ($kur_master) {
            ...
            $sr_ids = $kelas_gabungan->pluck('id_studentrecord')->toArray();
            if (!empty($sr_ids)) {
                $cross_records = DB::table('absensi_mahasiswa as am')
                    ...
                    ->whereIn('am.id_studentrecord', $sr_ids)
                    ...
                    ->select('am.id_studentrecord', 'k.kelas', 'd.nama as nama_dosen', 'b.pertemuan as sesi_pertemuan', 'am.keterangan')
                    ->get()
                    ->keyBy('id_studentrecord');

                foreach ($kelas_gabungan as $item) {
                    if (isset($cross_records[$item->id_studentrecord])) {
                        $cr = $cross_records[$item->id_studentrecord];
                        $item->hadir_di_kelas_lain = true;
                        $item->keterangan_kelas_lain = "Sudah Hadir di Kelas {$cr->kelas} (" . ($cr->nama_dosen ?: 'Dosen') . ") - Sesi P-{$cr->sesi_pertemuan}";
                    } else {
        ...
        // 4. Ambil mahasiswa lintas kelas yang sudah tersimpan pada BAP ini (jika ada)
        $registered_sr_ids = $kelas_gabungan->pluck('id_studentrecord')->toArray();
        $cross_students = DB::table('absensi_mahasiswa as am')
            ...
            ->join('kelas as f', 'kp.id_kelas', '=', 'f.idkelas')
            ->where('am.id_bap', $id)
            ->whereNotIn('am.id_studentrecord', $registered_sr_ids)
            ->select(
                'c.id_studentrecord',
                'c.id_kurperiode',
                'c.id_student',
    ...
    public function edit_absen($id_bap)
    {
        ...
        $kur_master = Kurikulum_periode::where('id_kurperiode', $id_kurperiode)->first();
        ...
        // Cek mahasiswa reguler yang hadir di kelas lain
        if ($kur_master) {
            $kur_gabungan_ids = DB::table('kurikulum_periode')
                ->where('id_periodetahun', $kur_master->id_periodetahun)
                ->where('id_periodetipe', $kur_master->id_periodetipe)
            ...
            $sr_ids = $data->pluck('id_studentrecord')->toArray();
            if (!empty($sr_ids)) {
                $cross_records = DB::table('absensi_mahasiswa as am')
                    ->join('bap as b', 'am.id_bap', '=', 'b.id_bap')
                    ->join('kurikulum_periode as kp', 'b.id_kurperiode', '=', 'kp.id_kurperiode')
        ...
        // Ambil mahasiswa lintas kelas yang sudah tersimpan di BAP ini
        $reg_ids = $data->pluck('id_studentrecord')->toArray();
        $cross_students = DB::table('absensi_mahasiswa as am')
            ->join('student_record as sr', 'am.id_studentrecord', '=', 'sr.id_studentrecord')
            ->join('student as mhs', 'sr.id_student', '=', 'mhs.idstudent')
            ->join('prodi as prd', function ($join) {
                $join->on('prd.kodeprodi', '=', 'mhs.kodeprodi')
                    ->on('prd.kodekonsentrasi', '=', 'mhs.kodekonsentrasi');
            })
            ->join('kurikulum_periode as kp', 'sr.id_kurperiode', '=', 'kp.id_kurperiode')
            ->join('kelas as kls', 'kp.id_kelas', '=', 'kls.idkelas')
            ->where('am.id_bap', $id_bap)
            ->whereNotIn('am.id_studentrecord', $reg_ids)
            ->select(
                'sr.id_studentrecord',
                'am.id_absensi',
                'am.id_bap',
    ...
    public function edit_bap($id)
    {
        ...
        return view('dosen/edit_bap', compact('id', 'bap', 'rps', 'jam', 'cek_mk'));
    }
    ...
    public function sum_absen($id)
    {
        ...
        // 2. Get ALL attendance records for this period in ONE query
        if (!empty($bap_ids)) {
        ...
        // 3. Map attendance to a structured array: [student_id][pertemuan] = status
        $attendance_map = [];
        $pertemuan_terisi = [];
        ...
        // 4. Attach attendance to student objects for easy view iteration
    ...
    public function download_absensi($id)
    {
        ...
        if (!empty($bap_ids)) {
            $abs = Absensi_mahasiswa::whereIn('absensi_mahasiswa.id_bap', $bap_ids)
                ->join('student_record', 'absensi_mahasiswa.id_studentrecord', '=', 'student_record.id_studentrecord')
                ->join('student', 'student_record.id_student', '=', 'student.idstudent')
                ->select('student_record.id_studentrecord', 'student.nama', 'student.nim', 'student.idstatus')
                ->distinct()
                ->orderBy('student.nim', 'ASC')
                ->get();
        ...
        $abs_filtered = $abs->filter(function ($item) use ($key) {
            return $item->idstatus == $key->id_kelas;
        })->values();

        if ($abs_filtered->isNotEmpty()) {
            $abs = $abs_filtered;
        }
        ...
        $pdf = PDF::loadView('dosen/download/absensi_perkuliahan_pdf', [
            'abs16' => $abs16, 'abs15' => $abs15, 'abs14' => $abs14, 'abs13' => $abs13,
            'abs12' => $abs12, 'abs11' => $abs11, 'abs10' => $abs10, 'abs9'  => $abs9,
            'abs8'  => $abs8,  'abs7'  => $abs7,  'abs6'  => $abs6,  'abs5'  => $abs5,
            'abs4'  => $abs4,  'abs'   => $abs,   'abs1'  => $abs1,  'abs2'  => $abs2,
            'abs3'  => $abs3,  'bap'   => $key
    ...
    public function pedoman_akademik_dsn_dlm(Request $request)
    {
        ...
        if ($request->ajax()) {
            $html = view('dosen.partials.pedoman_akademik_list', compact('pedoman'))->render();
        ...
        return view('dosen/pedoman_akademik', compact('pedoman', 'summary'));
    }
