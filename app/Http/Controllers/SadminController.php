...
use App\Models\Kuisioner_aspek;
use App\Models\Microsoft_user;
use App\Models\Wadir;
use App\Models\Wrkpersonalia;
...
use App\Models\Kalender_akademik;
use App\Models\Konversi_makul;
use App\Models\Pengajuan_kategori;
...
class SadminController extends Controller
{
    ...
    {
        Penangguhan_trans::where('id_penangguhan_trans', $id)->update(['validasi_baak' => 'SUDAH']);

        Alert::success('', 'Berhasil')->autoclose(3500);
        return redirect()->back();
    }

    public function batal_val_penangguhan_baak($id)
    {
        Penangguhan_trans::where('id_penangguhan_trans', $id)->update(['validasi_baak' => 'BELUM']);
    ...
    public function download_bap_dosen($id)
    {
        $bap = Kurikulum_periode::join('matakuliah', 'kurikulum_periode.id_makul', '=', 'matakuliah.idmakul')
            ->join('prodi', 'kurikulum_periode.id_prodi', '=', 'prodi.id_prodi')
            ->join('kelas', 'kurikulum_periode.id_kelas', '=', 'kelas.idkelas')
    ...
    public function download_absensi_mhs($id)
    {
        $bap = Kurikulum_periode::join('matakuliah', 'kurikulum_periode.id_makul', '=', 'matakuliah.idmakul')
            ->join('prodi', 'kurikulum_periode.id_prodi', '=', 'prodi.id_prodi')
            ->join('kelas', 'kurikulum_periode.id_kelas', '=', 'kelas.idkelas')
        ...
        if (!empty($bap_ids)) {
            ...
            $all_abs = Absensi_mahasiswa::whereIn('id_bap', $bap_ids)
        ...
        $abs_filtered = $abs->filter(function ($item) use ($bap) {
            return $item->idstatus == $bap->id_kelas;
        })->values();
        ...
        foreach ($abs as $student) {
            for ($i = 1; $i <= 16; $i++) {
                $prop = 'absn' . $i;
                $student->$prop = $attendance_map[$student->id_studentrecord][$i] ?? null;
            }
        }
    ...
    public function absen_edom()
    {
        $periodetahun = Periode_tahun::orderBy('id_periodetahun', 'DESC')->get();
        $periodetipe = Periode_tipe::orderBy('id_periodetipe', 'DESC')->get();

        return view('sadmin/edom/filter_absen_edom', compact('periodetahun', 'periodetipe'));
    }

    public function report_absen_edom(Request $request)
    {
