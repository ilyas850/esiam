...
class SadminController extends Controller
{
    ...
    {
      $akun = Periode_tahun::where('status', 'ACTIVE')
    ...
    {
      $akun = Periode_tipe::where('status', 'ACTIVE')
                          ->update(['status' => 'NOT ACTIVE']);
    ...
    public function data_nilai()
    {
      ...
      $nilai = student_record::join('student', 'student_record.id_student', '=', 'student.idstudent')
                            ...
                            //   ->where('student.idangkatan', 18)
                            //   ->where('student.kodeprodi', 24)
                              ->where('student.active', 1)
      ...
      return view('sadmin/data_nilai', ['nilai'=>$nilai, 'mk'=>$makul]);
    }
    ...
    public function cek_nilai(Request $request)
    {
      ...
      return view ('sadmin/ceknilai', ['cek'=>$cek, 'key'=>$key, 'mk'=>$makul, 'mhs'=>$mhs]);
    }
...
dd($request);
    ...
    public function downloadktm($id)
    {
      ...
      $pdf= PDF::loadView('sadmin/ktm', ['mhs'=>$keymhs, 'prd'=>$prd])->setPaper('a4', 'landscape');
  ...
  public function post_kaprodi(Request $request)
  {
    ...
    $akun = User::where('id_user', $request->id_dosen)
                            ->update(['role' => 6]);
  ...
  {
    $akun = Kaprodi::where('id_kaprodi', $request->id_kaprodi)
                            ->update(['status' => 'NOT ACTIVE']);
    ...
    public function lihat_transkrip(Request $request)
    {
      ...
      $cek = Transkrip_nilai::where('no_transkrip', $nomor)
                            ->get();
      if (count($cek) > 0) {

        Alert::error('Nomor surat sudah terdaftar', 'MAAF !!');
        return redirect()->back();
      }elseif (count($cek) == 0) {
