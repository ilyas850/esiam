...
class KaprodiController extends Controller
{
  public function kaprodi_home()
  {
    ...
    $data_akademik = $data_akademik_collection->map(function ($item) {
      ...
      $obj->sks = (float) $item->makul->akt_sks_teori + (float) $item->makul->akt_sks_praktek;
  ...
  function entri_rps($id)
  {
    ...
    return view('kaprodi/pengajaran/form_rps', compact('id', 'data', 'pertemuan'));
  }
  ...
  function edit_rps($id)
  {
    ...
    return view('kaprodi/pengajaran/edit_form_rps', compact('id', 'data', 'pertemuan', 'rps'));
  }
  ...
  public function unduh_pdf_nilai($id)
  {
    ...
    $pdf = PDF::loadView('kaprodi/matakuliah/unduh_nilai_pdf', [
      'd' => $d,
      'm' => $m,
      'y' => $y,
      'data' => $key,
  ...
  public function entri_bap($id)
  {
    ...
    return view('kaprodi/bap/bap', compact('data', 'bap'));
  }
  ...
  public function save_bap(Request $request)
  {
    ...
    return redirect('entri_bap_kprd/' . $id_kurperiode)->with('success', 'Data Berhasil diupload');
  }
  ...
  public function edit_absen($id)
  {
    ...
    $data = DB::table('student_record as sr')
      ...
      ->leftJoin('absensi_mahasiswa as am', function ($join) use ($id) {
      ...
      ->whereExists(function ($query) use ($idk) {
        $query->select(DB::raw(1))
          ->from('kurikulum_periode as kp_inner')
          ->where('kp_inner.id_kurperiode', '=', $idk)
  ...
  public function sum_absen($id)
  {
    ...
    $nama_dosen_2 = $dosen2 ? ($dosen2->nama . ', ' . $dosen2->akademik) : '';

    $bap_records = Bap::where('id_kurperiode', $id)
      ->where('status', 'ACTIVE')
      ->select('id_bap', 'pertemuan')
      ->get();

    $bap_ids = $bap_records->pluck('id_bap')->toArray();
    $bap_pertemuan_map = $bap_records->pluck('pertemuan', 'id_bap')->toArray();

    if (!empty($bap_ids)) {
      $students = Absensi_mahasiswa::whereIn('absensi_mahasiswa.id_bap', $bap_ids)
      ...
      $all_attendance = Absensi_mahasiswa::whereIn('id_bap', $bap_ids)
        ->select('id_studentrecord', 'absensi', 'id_bap')
        ->get();
    } else {
      $students = Student_record::join('student', 'student_record.id_student', '=', 'student.idstudent')
        ->where('student_record.id_kurperiode', $id)
        ->where('student_record.status', 'TAKEN')
        ->select('student_record.id_studentrecord', 'student.nama', 'student.nim')
        ->orderBy('student.nim')
    ...
    foreach ($all_attendance as $rec) {
      $pertemuan = $bap_pertemuan_map[$rec->id_bap] ?? null;
      if ($pertemuan) {
        $attendance_map[$rec->id_studentrecord][$pertemuan] = $rec->absensi;
        $pertemuan_terisi[$pertemuan] = true;
      }
    }
    ...
    $summary = [
      ...
      'total_pertemuan_terisi' => count($pertemuan_terisi),
      'total_hadir' => $all_attendance->where('absensi', 'ABSEN')->count(),
      'total_tidak_hadir' => $all_attendance->whereIn('absensi', ['SAKIT', 'IZIN', 'ALFA'])->count(),
    ];

    return view('kaprodi/bap/absensi_perkuliahan', [
      'bap' => $bap,
      'nama_dosen_2' => $nama_dosen_2,
  ...
  public function sum_absen_his($id)
  {
    ...
    if (!empty($bap_ids)) {
      $abs = Absensi_mahasiswa::whereIn('absensi_mahasiswa.id_bap', $bap_ids)
        ->join('student_record', 'absensi_mahasiswa.id_studentrecord', '=', 'student_record.id_studentrecord')
        ->join('student', 'student_record.id_student', '=', 'student.idstudent')
        ->select('student_record.id_studentrecord', 'student.nama', 'student.nim')
        ->distinct()
      ...
      $all_abs = Absensi_mahasiswa::whereIn('absensi_mahasiswa.id_bap', $bap_ids)
        ->join('bap', 'absensi_mahasiswa.id_bap', '=', 'bap.id_bap')
        ->select('absensi_mahasiswa.absensi', 'absensi_mahasiswa.id_studentrecord', 'bap.pertemuan')
        ->get()
        ->groupBy('pertemuan');
    } else {
      $abs = Student_record::join('student', 'student_record.id_student', '=', 'student.idstudent')
        ->where('student_record.id_kurperiode', $id)
        ->where('student_record.status', 'TAKEN')
        ->select('student_record.id_studentrecord', 'student.nama', 'student.nim')
        ->orderBy('student.nim')
    ...
    $abs1  = $all_abs->get(1, collect());
    $abs2  = $all_abs->get(2, collect());
    $abs3  = $all_abs->get(3, collect());
    $abs4  = $all_abs->get(4, collect());
    $abs5  = $all_abs->get(5, collect());
    $abs6  = $all_abs->get(6, collect());
    $abs7  = $all_abs->get(7, collect());
    $abs8  = $all_abs->get(8, collect());
    $abs9  = $all_abs->get(9, collect());
    $abs10 = $all_abs->get(10, collect());
    $abs11 = $all_abs->get(11, collect());
    $abs12 = $all_abs->get(12, collect());
    $abs13 = $all_abs->get(13, collect());
    $abs14 = $all_abs->get(14, collect());
    $abs15 = $all_abs->get(15, collect());
    $abs16 = $all_abs->get(16, collect());
  ...
  public function val_soal_uas_kprd()
  {
    $iduser = Auth::user()->id_user;
    ...
    return view('kaprodi/master/cek_soal_uas', compact('data'));
  }

  public function komentar_soal_uas(Request $request, $id)
  {
    $prd = Soal_ujian::find($id);
    $prd->komentar_uas = $request->komentar_uas;
    $prd->save();
  ...
  public function val_soal_uas($id)
  {
    $val = Soal_ujian::find($id);
    $val->validasi_uas = 'SUDAH';
    $val->save();
  ...
  public function input_partisipatif_kprd($id)
  {
    $kelas_gabungan = Helper::getMahasiswaDosen($id);
    return view('kaprodi/matakuliah/input_partisipatif_dsn', ['kuri' => $id, 'ck' => $kelas_gabungan, 'id' => $id]);
  }

  public function save_nilai_partisipatif_kprd(Request $request)
  {
    ...
    return redirect('/cekmhs_dsn_kprd/' . $request->id_kurperiode);
  }

  public function input_proyek_kprd($id)
  {
    $kelas_gabungan = Helper::getMahasiswaDosen($id);
    return view('kaprodi/matakuliah/input_proyek_dsn', ['kuri' => $id, 'ck' => $kelas_gabungan, 'id' => $id]);
  }

  public function save_nilai_proyek_kprd(Request $request)
  {
  ...
  public function input_tugas_kprd($id)
  {
    $kelas_gabungan = Helper::getMahasiswaDosen($id);
    return view('kaprodi/matakuliah/input_tugas_dsn', ['kuri' => $id, 'ck' => $kelas_gabungan, 'id' => $id]);
  }

  public function save_nilai_tugas_kprd(Request $request)
  {
  ...
  public function input_kuis_kprd($id)
  {
    $kelas_gabungan = Helper::getMahasiswaDosen($id);
    return view('kaprodi/matakuliah/input_kuis_dsn', ['kuri' => $id, 'ck' => $kelas_gabungan, 'id' => $id]);
  }

  public function save_nilai_kuis_kprd(Request $request)
  {
  ...
  {
    Prausta_setting_relasi::where('id_settingrelasi_prausta', $request->id_settingrelasi_prausta)->update(['acc_judul_dospem' => $request->acc_judul]);
  ...
  {
    Prausta_setting_relasi::where('id_settingrelasi_prausta', $id)->update(['acc_seminar_sidang' => 'TERIMA']);
  ...
  public function jadwal_prausta_kprd()
  {
    $id = Auth::user()->id_user;
    ...
    return view('kaprodi/prausta/jadwal_prausta', compact('data'));
  }

  function jadwal_magang_skripsi_kprd()
  {
    $id = Auth::user()->id_user;
    ...
    return view('kaprodi/magang_skripsi/jadwal_magang_skripsi', compact('data'));
  }
  ...
  public function generate_nilai_akhir_dsn_kprd(Request $request)
  {
    ...
    if (!$set_nilai) {
      ...
      return redirect('/cekmhs_dsn_kprd/' . $idkur);
    }
  ...
  public function krs_mahasiswa_kprd(Request $request)
  {
    $iddosen = Auth::user()->id_user;
    $perPage = (int) $request->get('per_page', 10);
    ...
    $krsSummary = Student_record::join('kurikulum_periode', 'student_record.id_kurperiode', '=', 'kurikulum_periode.id_kurperiode')
      ->where('student_record.status', 'TAKEN')
      ->where('kurikulum_periode.status', 'ACTIVE')
      ->groupBy('student_record.id_student')
      ->select('student_record.id_student', DB::raw('COUNT(student_record.id_student) as jml_krs'));
    ...
    $data = Student::leftJoin('prodi', function ($join) {
      ...
      ->join('kelas', 'student.idstatus', '=', 'kelas.idkelas')
      ->join('angkatan', 'student.idangkatan', '=', 'angkatan.idangkatan')
      ->leftJoinSub($krsSummary, 'krs_summary', function ($join) {
        $join->on('student.idstudent', '=', 'krs_summary.id_student');
      })
      ->where('student.active', 1)
      ->where('student.kodeprodi', $prodi->kodeprodi)
      ->when($request->q, function ($query) use ($request) {
        $query->where(function ($subQuery) use ($request) {
          $subQuery->where('student.nama', 'like', '%' . $request->q . '%')
            ->orWhere('student.nim', 'like', '%' . $request->q . '%');
        });
      })
      ->select(
        'student.idstudent',
      ...
      ->selectRaw('COALESCE(krs_summary.jml_krs, 0) as jml_krs')
      ->orderBy('student.nim', 'ASC')
      ->paginate($perPage)
      ->appends($request->only('q', 'per_page'));

    if ($request->ajax()) {
      $html = view('kaprodi.master.partials.cek_krs_mhs_prodi_table', compact('data'))->render();
      return response()->json(['html' => $html]);
    }
  ...
  function pembimbing_magang2_kprd()
  {
    $id = Auth::user()->id_user;
    ...
    return view('kaprodi/magang_skripsi/pembimbing_magang2', compact('data'));
  }
  ...
  public function record_bim_magang2($id)
  {
    ...
    return view('kaprodi/magang_skripsi/cek_bimbingan_magang2', compact('jdl', 'pkl'));
  }
  ...
  public function penguji_magang_kprd()
  {
    $id = Auth::user()->id_user;
    ...
    return view('kaprodi/magang_skripsi/penguji_magang', compact('data'));
  }

  public function penguji_magang2_kprd()
  {
    $id = Auth::user()->id_user;
    ...
    return view('kaprodi/magang_skripsi/penguji_magang2', compact('data'));
  }

  public function isi_form_nilai_magang_kaprodi($id)
  {
    $data = Prausta_setting_relasi::join('student', 'prausta_setting_relasi.id_student', '=', 'student.idstudent')
      ->where('prausta_setting_relasi.id_settingrelasi_prausta', $id)
      ->select('prausta_setting_relasi.id_settingrelasi_prausta', 'student.nama', 'student.nim', 'prausta_setting_relasi.tempat_prausta')
    ...
    return view('kaprodi/magang_skripsi/form_nilai_magang', compact('data', 'id', 'form_dosbing', 'form_seminar'));
  }

  public function isi_form_nilai_magang2_kaprodi($id)
  {
    $data = Prausta_setting_relasi::join('student', 'prausta_setting_relasi.id_student', '=', 'student.idstudent')
      ->where('prausta_setting_relasi.id_settingrelasi_prausta', $id)
      ->select('prausta_setting_relasi.id_settingrelasi_prausta', 'student.nama', 'student.nim', 'prausta_setting_relasi.tempat_prausta')
    ...
    return view('kaprodi/magang_skripsi/form_nilai_magang2', compact('data', 'id', 'form_dosbing', 'form_seminar'));
  }

  public function simpan_nilai_magang_kaprodi(Request $request)
  {
    $id_prausta = $request->id_settingrelasi_prausta;
    $nilai_pem_lap = $request->nilai_pembimbing_lapangan;
    $id_penilaian1 = $request->id_penilaian_prausta1;
    ...
    return redirect('penguji_magang_kprd');
  }

  public function simpan_nilai_magang2_kaprodi(Request $request)
  {
    $id_prausta = $request->id_settingrelasi_prausta;
    $nilai_pem_lap = $request->nilai_pembimbing_lapangan;
    $id_penilaian1 = $request->id_penilaian_prausta1;
    ...
    Alert::success('', 'Nilai Magang berhasil disimpan')->autoclose(3500);
    return redirect('penguji_magang2_kprd');
  }

  public function edit_nilai_magang_by_kaprodi($id)
  {
    $datadiri = Prausta_setting_relasi::join('student', 'prausta_setting_relasi.id_student', '=', 'student.idstudent')
      ->where('prausta_setting_relasi.id_settingrelasi_prausta', $id)
      ->select('prausta_setting_relasi.id_settingrelasi_prausta', 'student.nama', 'student.nim', 'prausta_setting_relasi.tempat_prausta')
    ...
    return view('kaprodi/magang_skripsi/edit_nilai_magang', compact('nilai_pem', 'datadiri', 'nilai_sem', 'id', 'nilai_1'));
  }

  public function edit_nilai_magang2_by_kaprodi($id)
  {
    $datadiri = Prausta_setting_relasi::join('student', 'prausta_setting_relasi.id_student', '=', 'student.idstudent')
      ->where('prausta_setting_relasi.id_settingrelasi_prausta', $id)
      ->select('prausta_setting_relasi.id_settingrelasi_prausta', 'student.nama', 'student.nim', 'prausta_setting_relasi.tempat_prausta')
    ...
    return view('kaprodi/magang_skripsi/edit_nilai_magang2', compact('nilai_pem', 'datadiri', 'nilai_sem', 'id', 'nilai_1'));
  }

  public function put_nilai_magang_by_kaprodi(Request $request)
  {
    $id_prausta = $request->id_settingrelasi_prausta;
    $nilai_pem_lap = $request->nilai_pembimbing_lapangan;
    $id_penilaian1 = $request->id_penilaian_prausta1;
  ...
  public function put_nilai_magang2_by_kaprodi(Request $request)
  {
    $id_prausta = $request->id_settingrelasi_prausta;
    $nilai_pem_lap = $request->nilai_pembimbing_lapangan;
    $id_penilaian1 = $request->id_penilaian_prausta1;
  ...
  public function pembimbing_sempro_skripsi_kprd()
  {
    $id = Auth::user()->id_user;
    ...
    return view('kaprodi/magang_skripsi/pembimbing_sempro', compact('data'));
  }

  public function record_bim_sempro_skripsi_kprd($id)
  {
    $jdl = Prausta_setting_relasi::join('student', 'prausta_setting_relasi.id_student', '=', 'student.idstudent')
      ->leftJoin('prodi', function ($join) {
        $join->on('prodi.kodeprodi', '=', 'student.kodeprodi')->on('prodi.kodekonsentrasi', '=', 'student.kodekonsentrasi');
    ...
    return view('kaprodi/magang_skripsi/cek_bimbingan_sempro', compact('jdl', 'pkl'));
  }

  public function komentar_bimbingan_sempro_skripsi_kprd(Request $request, $id)
  {
    $prd = Prausta_trans_bimbingan::find($id);
    $prd->komentar_bimbingan = $request->komentar_bimbingan;
    $prd->save();
