    ...
    <section class="content">
        <div class="box box-danger">
            <div class="box-header with-border">
                <h3 class="box-title">Filter Report Kuisioner Dosen Pembimbing TA</h3>
            </div>
            <form class="form" action="{{ url('post_report_kuisioner_dsn_ta_gugusmutu') }}" method="POST">
                {{ csrf_field() }}
        ...
        <div class="box box-info">
            <div class="box-header">
                <h3 class="box-title">Report Kuisioner Dosen Pembimbing TA <b> {{ $namaperiodetahun }} -
                        {{ $namaperiodetipe }} </b></h3>
            </div>
            <div class="box-body">
                <form action="{{ url('download_kuisioner_dsn_ta_gugusmutu') }}" method="POST">
                    {{ csrf_field() }}
                    <input type="hidden" name="id_kategori_kuisioner" value="{{ $id }}">
                    ...
                    <tbody>
                        ...
                        @foreach ($data as $item)
                            <tr>
                                ...
                                <td>
                                    <center>
                                        <form action="{{ url('detail_kuisioner_dsn_ta_gugusmutu') }}" method="POST">
                                            {{ csrf_field() }}
                                ...
                                <td>
                                    <center>
                                        <form action="{{ url('download_detail_kuisioner_dsn_ta_gugusmutu') }}" method="POST">
                                            {{ csrf_field() }}
                                            <input type="hidden" name="id_dosen" value="{{ $item->iddosen }}">
                                            <input type="hidden" name="id_periodetahun" value="{{ $idperiodetahun }}">
                                            <input type="hidden" name="id_periodetipe" value="{{ $idperiodetipe }}">
