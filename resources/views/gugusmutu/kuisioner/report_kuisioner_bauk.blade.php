    ...
    <section class="content">
        <div class="box box-danger">
            <div class="box-header with-border">
                <h3 class="box-title">Filter Report Kuisioner BAUK</h3>
            </div>
            <form class="form" action="{{ url('post_report_kuisioner_bauk_gugusmutu') }}" method="POST">
                {{ csrf_field() }}
        ...
        <div class="box box-info">
            <div class="box-header">
                <h3 class="box-title">Report Kuisioner BAUK <b> {{ $namaperiodetahun }} -
                        {{ $namaperiodetipe }} </b></h3>
            </div>
            <div class="box-body">
                <form action="{{ url('download_kuisioner_bauk_gugusmutu') }}" method="POST">
                    {{ csrf_field() }}
                    <input type="hidden" name="id_kategori_kuisioner" value="{{ $id }}">
                    ...
                    <tbody>
                        ...
                        @foreach ($data as $item)
                            <tr>
                                ...
                                <td>
                                    <center>
                                        <form action="{{ url('detail_kuisioner_bauk_gugusmutu') }}" method="POST">
                                            {{ csrf_field() }}
                                ...
                                <td>
                                    <center>
                                        <form action="{{ url('download_detail_kuisioner_bauk_gugusmutu') }}" method="POST">
                                            {{ csrf_field() }}
                                            <input type="hidden" name="kodeprodi" value="{{ $item->kodeprodi }}">
                                            <input type="hidden" name="id_periodetahun" value="{{ $idperiodetahun }}">
                                            <input type="hidden" name="id_periodetipe" value="{{ $idperiodetipe }}">
