    ...
    <section class="content">
        <div class="box box-info">
            <div class="box-header">
                <h3 class="box-title">Form Pengajuan Pindah Kelas</h3>
            </div>
            <div class="box-body">
                <div class="row">
                    <div class="col-xs-2">
                        <button type="button" class="btn btn-success mr-5" data-toggle="modal" data-target="#addpindahkelas">
                            Input Data Pindah Kelas
                        </button>
                    </div>
                </div>
                <br>
                <div class="modal fade" id="addpindahkelas" tabindex="-1" role="dialog"
                    aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <form method="post" action="{{ url('post_pindah_kelas') }}" enctype="multipart/form-data">
                            {{ csrf_field() }}
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control" value="{{ $datamhs->kelas }}"
                                                    readonly>
                                                <input type="hidden" name="kelas_sebelum" value="{{ $datamhs->idstatus }}">
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group">
                    ...
                    <tbody>
                        ...
                        @foreach ($data as $item)
                            <tr>
                                ...
                                <td align="center">
                                    @foreach ($kelas as $kls)
                                        @if ($item->kelas_sebelum == $kls->idkelas)
                                            {{ $kls->kelas }}
                                        @endif
                                    @endforeach
                                </td>
                                <td align="center">
                                    @foreach ($kelas as $kls)
                                        @if ($item->kelas_tujuan == $kls->idkelas)
                                            {{ $kls->kelas }}
                                        @endif
                                    @endforeach
                                </td>
                            ...
                            <div class="modal fade" id="modalUpdateKelas{{ $item->id_trans_pengajuan }}" tabindex="-1"
                                aria-labelledby="modalUpdateKelas" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title">Update Pengajuan Pindah Kelas</h5>
                                        </div>
                                        <form action="/put_pengajuan_pindah_kelas/{{ $item->id_trans_pengajuan }}"
                                            ...
                                            <div class="modal-body">
                                                <div class="row">
                                                    <div class="col-md-6">
                                                        <div class="form-group">
                                                            <label>Pindah Kelas Pada Tahun Akademik</label>
                                                            <select name="id_periodetahun" class="form-control" required>
                                                    ...
                                                    <div class="col-md-6">
                                                        <div class="form-group">
                                                            ...
                                                            <input type="text" class="form-control"
                                                                value="{{ $datamhs->kelas }}" readonly>
                                                            <input type="hidden" name="kelas_sebelum"
                                                                value="{{ $item->kelas_sebelum }}">
                                                        </div>
                                                    </div>
                                                    <div class="col-md-6">
                                                        <div class="form-group">
                                                            <label>Kelas Tujuan</label>
                                                            <select name="kelas_tujuan" class="form-control" required>
                                                                <option value="{{ $item->kelas_tujuan }}">
                                                                    @foreach ($kelas as $kls)
                                                                        @if ($item->kelas_tujuan == $kls->idkelas)
                                                                            {{ $kls->kelas }}
                                                                        @endif
