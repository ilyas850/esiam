...
@section('content')
    <section class="content">
        ...
        <div class="box box-info">
            ...
            <div class="box-body">
                <a href="/input_bap_kprd/{{$bap->id_kurperiode}}" class="btn btn-success">Input BAP</a>
                <a href="/sum_absen_kprd/{{$bap->id_kurperiode}}" class="btn btn-info">Absensi Perkuliahan</a>
                <a href="/jurnal_bap_kprd/{{$bap->id_kurperiode}}" class="btn btn-warning">Jurnal Perkuliahan</a>
                <br><br>
                <table class="table table-bordered table-striped">
                    <thead>
                        <tr>
                            ...
                            <th rowspan="2"><center>Kurang Jam</center></th>
                            ...
                            <th rowspan="2"><center>Absen</center></th>
                    ...
                    <tbody>
                        @foreach ($data as $item)
                            <tr>
                                ...
                                <td><center>{{$item->kurang_jam}}</center></td>
                                ...
                                <td><center>
                                    @if ($item->hadir != null && $item->tidak_hadir != null )
                                        <a href="/edit_absen_kprd/{{$item->id_bap}}" class="btn btn-success btn-xs"> Edit</a>
                                    @elseif ($item->hadir == null && $item->tidak_hadir == null)
                                        <a href="/entri_absen_kprd/{{$item->id_bap}}" class="btn btn-warning btn-xs"> Entri</a>
                                    @endif
                                </center></td>
                                <td><center>
                                    <a href="/view_bap_kprd/{{$item->id_bap}}" class="btn btn-info btn-xs" title="klik untuk lihat"> <i class="fa fa-eye"></i></a>
                                    @if ($item->tanggal_validasi != null)
                                        <span class="badge bg-yellow">Valid</span>
                                    @elseif($item->tanggal_validasi == null)
                                        <a href="/edit_bap_kprd/{{$item->id_bap}}" class="btn btn-success btn-xs" title="klik untuk edit"> <i class="fa fa-edit"></i></a>
                                        <a href="/delete_bap_kprd/{{$item->id_bap}}" class="btn btn-danger btn-xs" title="klik untuk hapus"> <i class="fa fa-trash"></i></a>
                                    @endif
                                </center></td>
                            </tr>
                        @endforeach
