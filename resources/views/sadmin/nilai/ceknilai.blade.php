      ...
      <form action="{{url('save_nilai_angka')}}" method="post">
        ...
        <div class="box-body">
          <table  class="table table-bordered table-striped">
            <thead>
            <tr>
              <th rowspan="2" width="4px"><center>No</center></th>

              <th rowspan="2"><center>Matakuliah</center></th>
              <th colspan="2"><center>SKS</center></th>
              <th rowspan="2"><center>Nilai Huruf</center></th>
              <th rowspan="2"><center>Nilai Angka</center></th>
              <th rowspan="2"><center>Pilih</center></th>
            </tr>
            <tr>
            ...
            <tbody>
              ...
              @foreach ($cek as $key)
                <tr>
                  <td>{{$no++}}</td>
                  <td>{{$key->makul}}</td>
                  <td><center>{{$key->akt_sks_teori}}</center></td>
                  <td><center>{{$key->akt_sks_praktek}}</center></td>
                  <td><center>{{$key->nilai_AKHIR}}</center></td>
                  <td><center>{{$key->nilai_ANGKA}}</center></td>
                  <td>
                    @if ($key->nilai_ANGKA==null)
                          <center><input type="checkbox" name="nilai_ANGKA[]" value="{{$key->id_studentrecord}},
                            @if ($key->nilai_AKHIR=='A')
                              4
