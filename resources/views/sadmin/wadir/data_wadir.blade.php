                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="id_dosen">
                                  ...
                                  @foreach ($dosen as $keydsn)
                                    <option value="{{$keydsn->iddosen}},{{$keydsn->nama}}">{{$keydsn->nama}}</option>
                                  @endforeach
                                </select>
                              </div>
                           <div class="form-group">
          ...
          <tbody>
            ...
            @foreach ($wadir as $key)
              <tr>
                ...
                <td><center>{{$key->nik}}</center></td>
                <td><center>{{$key->nama}}</center></td>
                <td><center>{{$key->wadir}}</center></td>
                <td><center>
                  <button class="btn btn-success btn-xs" data-toggle="modal" data-target="#modalUpdateKaprodi{{ $key->id_kaprodi }}" title="klik untuk edit"><i class="fa fa-edit"></i></button>
                  <button class="btn btn-danger btn-xs" data-toggle="modal" data-target="#modalHapusKaprodi{{ $key->id_kaprodi }}" title="klik untuk hapus"><i class="fa fa-trash"></i></button>
                </center></td>
              </tr>

              <div class="modal fade" id="modalUpdateKaprodi{{ $key->id_kaprodi }}" tabindex="-1" aria-labelledby="modalUpdateKaprodi" aria-hidden="true">
                  <div class="modal-dialog">
                      <div class="modal-content">
                          <div class="modal-header">
                          ...
                          <form action="/put_kaprodi/{{ $key->id_kaprodi }}" method="post">
                              ...
                              <div class="form-group">
                                  <label>Nama Dosen</label>
                                  <select class="form-control" name="id_dosen">
                                    <option value="{{$key->id_dosen}}">{{$key->nama}}</option>
                                    @foreach ($dosen as $keydsn)
              ...
              <div class="modal fade" id="modalHapusKaprodi{{ $key->id_kaprodi }}" tabindex="-1" aria-labelledby="modalHapusKaprodi" aria-hidden="true">
                  <div class="modal-dialog">
                      <div class="modal-content">
                      <div class="modal-body">
