            ...
            <div class="col-xs-12">
                <div class="box box-primary">
                    <div class="box-header with-border">
                        <h3 class="box-title"><i class="fa fa-user-md"></i> Daftar Dosen Tetap ({{ count($dosenWithMakul) }}
                            orang)</h3>
                    </div>
                    <div class="box-body">
                                                ...
                                                @if(count($item['matakuliah']) > 0)
                                                    <ul style="margin: 0; padding-left: 20px;">
                                                        @foreach($item['matakuliah'] as $mk)
                                                            <li>{{ $mk['namakul'] }} ({{ $mk['sks_teori'] + $mk['sks_praktek'] }} SKS) - Kelas: {{ $mk['kelas'] }} | Prodi: {{ $mk['prodi'] }}</li>
                                                        @endforeach
                                                    </ul>
                                                @else
