...
@section('content')
    <section class="content-header">
        <h1>
            Validasi PKL & Magang
            <small>Validasi Akhir BAAK Mahasiswa</small>
        </h1>
        <ol class="breadcrumb">
            <li><a href="{{ url('home') }}"><i class="fa fa-dashboard"></i> Dashboard</a></li>
            <li><a href="#">PraUSTA</a></li>
            <li class="active">Validasi PKL</li>
        </ol>
    </section>
    ...
    <section class="content">
        ...
        <div class="nav-tabs-custom">
            <ul class="nav nav-tabs">
                <li class="active">
                    <a href="{{ url('data_val_pkl_mahasiswa') }}">
                        <i class="fa fa-briefcase text-blue"></i> <b>Validasi PKL</b>
                        <span class="label label-primary" style="margin-left: 5px;">{{ $total }}</span>
                    </a>
                </li>
                <li>
                    <a href="{{ url('data_val_magang_mahasiswa') }}">
                        <i class="fa fa-building text-green"></i> <b>Validasi Magang 1</b>
                    </a>
                </li>
                <li>
                    <a href="{{ url('data_val_magang2_mahasiswa') }}">
                        <i class="fa fa-building-o text-orange"></i> <b>Validasi Magang 2</b>
                    </a>
                </li>
            </ul>
                                            ...
                                            @if (!empty($item->file_laporan_revisi))
                                                <a href="{{ url('/File Laporan Revisi/' . $item->idstudent . '/' . $item->file_laporan_revisi) }}"
                                                   target="_blank" class="btn btn-default btn-xs btn-flat" style="border-color: #ccc;" title="Buka File Laporan">
                                                    <i class="fa fa-file-pdf-o text-red"></i> File Laporan
                                                </a>
                                            @else
                                        ...
                                        <td style="text-align: center; vertical-align: middle;">
                                            @if ($item->validasi_baak == 'BELUM')
                                                @if (!empty($item->nilai_huruf))
                                                    <button type="button" class="btn btn-info btn-xs btn-flat btn-confirm-action"
                                                            ...
                                                            data-title="Konfirmasi Validasi PKL"
                                                            data-message="Apakah Anda yakin ingin memvalidasi akhir PKL mahasiswa: <br><b>{{ $item->nama }} ({{ $item->nim }})</b>?"
                                            ...
                                            @elseif ($item->validasi_baak == 'SUDAH')
                                                <button type="button" class="btn btn-danger btn-xs btn-flat btn-confirm-action"
                                                        ...
                                                        data-title="Konfirmasi Batal Validasi PKL"
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#check-all').on('change', function() {
                ...
                // Hanya check checkbox baris yang aktif dan sedang terlihat (visible) setelah filtering
            ...
            // Modal Single Action Confirmation
