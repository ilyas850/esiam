        ...
        <div class="box box-info">
            ...
            <div class="box-body">
                <div style="margin-bottom: 15px;">
                    ...
                    @else
                        ...
                        @if ((float)$nilai->partisipatif > 0)
                            <a href="/input_partisipatif_dsn/{{ $ids }}" class="btn btn-primary btn-sm btn-flat">
                                <i class="fa fa-pencil"></i> Input Partisipatif ({{ $nilai->partisipatif }}%)
                            </a>
                        @endif

                        @if ((float)$nilai->proyek > 0)
                            <a href="/input_proyek_dsn/{{ $ids }}" class="btn btn-success btn-sm btn-flat">
                                <i class="fa fa-pencil"></i> Input Proyek ({{ $nilai->proyek }}%)
                            </a>
                        @endif

                        @if ((float)$nilai->tugas > 0)
                            <a href="/input_tugas_dsn/{{ $ids }}" class="btn btn-info btn-sm btn-flat">
                                <i class="fa fa-pencil"></i> Input Tugas ({{ $nilai->tugas }}%)
                            </a>
                        @endif

                        @if ((float)$nilai->kuis > 0)
                            <a href="/input_kuis_dsn/{{ $ids }}" class="btn btn-warning btn-sm btn-flat">
                        ...
                        @if (!$is_obe && (float)$nilai->kat > 0)
                            <a href="/input_kat_dsn/{{ $ids }}" class="btn btn-success btn-sm btn-flat">
                                <i class="fa fa-pencil"></i> Input Nilai KAT ({{ $nilai->kat }}%)
                            </a>
                        @endif

                        @if ((float)$nilai->uts > 0)
                            <a href="/input_uts_dsn/{{ $ids }}" class="btn bg-purple btn-sm btn-flat">
                                <i class="fa fa-pencil"></i> Input Nilai UTS ({{ $nilai->uts }}%)
                            </a>
                        @endif

                        @if ((float)$nilai->uas > 0)
                            <a href="/input_uas_dsn/{{ $ids }}" class="btn bg-maroon btn-sm btn-flat">
                ...
                @if ($nilai != null)
                    <div class="modal fade" id="editsettingnilai{{ $nilai->id_settingnilai }}" tabindex="-1" role="dialog"
                        ...
                        <div class="modal-dialog modal-lg">
                            <form method="post" action="/put_settingnilai_dsn_dlm/{{ $nilai->id_settingnilai }}" class="form-setting-nilai">
                                @csrf
                                @method('put')
                ...
                <div class="modal fade" id="addsettingnilai" tabindex="-1" role="dialog"
                    ...
                    <div class="modal-dialog modal-lg">
                        <form method="post" action="{{ url('post_settingnilai_dsn_dlm') }}" class="form-setting-nilai">
                            @csrf
                ...
                <div class="modal modal-danger fade" id="modal-danger">
                    <div class="modal-dialog">
                        <form action="{{ url('generate_nilai_akhir_dsn_dlm') }}" method="POST">
                            @csrf
                            <input type="hidden" name="id_kurperiode" value="{{ $ids }}">
                            <div class="modal-content">
                                <div class="modal-header">
