<ul class="sidebar-menu" data-widget="tree">
    <li class="header">Menu PraUSTA</li>
    <li><a href="{{ url('home') }}"><i class="fa fa-dashboard"></i> <span>Halaman Utama</span></a></li>
    <li><a href="{{ url('nilai_prausta') }}"><i class="fa fa-list-alt"></i> <span>Nilai Hasil
                PraUSTA</span></a></li>
    <li class="treeview">
        <a href="#">
            <i class="fa fa-database"></i> <span>Setting PraUSTA</span>
            <span class="pull-right-container">
                <i class="fa fa-angle-left pull-right"></i>
            </span>
        </a>
        <ul class="treeview-menu">
            <li><a href="{{ url('data_pkl_magang') }}"><i class="fa fa-circle-o"></i> <span>PKL/Magang</span></a></li>
            <li><a href="{{ url('data_sempro') }}"><i class="fa fa-circle-o"></i> <span>Seminar Proposal</span></a>
            </li>
            <li><a href="{{ url('data_ta_skripsi') }}"><i class="fa fa-circle-o"></i> <span>TA/Skripsi</span></a></li>
        </ul>
    </li>
    <li class="treeview">
        <a href="#">
            <i class="fa fa-database"></i> <span>Bimbingan PraUSTA</span>
            <span class="pull-right-container">
                <i class="fa fa-angle-left pull-right"></i>
            </span>
        </a>
        <ul class="treeview-menu">
            <li><a href="{{ url('bim_pkl_magang') }}"><i class="fa fa-circle-o"></i> <span>
                PKL/Magang</span></a></li>
            <li><a href="{{ url('bim_sempro') }}"><i class="fa fa-circle-o"></i> <span>
                        Seminar Proposal</span></a></li>
            <li><a href="{{ url('bim_ta_skripsi') }}"><i class="fa fa-circle-o"></i> <span>
                        TA/Skripsi</span></a></li>
        </ul>
    </li>
    <li class="treeview">
        <a href="#">
            <i class="fa fa-database"></i> <span>Nilai PraUSTA</span>
            <span class="pull-right-container">
                <i class="fa fa-angle-left pull-right"></i>
            </span>
        </a>
        <ul class="treeview-menu">
            <li><a href="{{ url('nilai_pkl_magang') }}"><i class="fa fa-circle-o"></i> <span>
                        PKL/Magang</span></a></li>
            <li><a href="{{ url('nilai_sempro') }}"><i class="fa fa-circle-o"></i> <span>
                        Seminar Proposal</span></a></li>
            <li><a href="{{ url('nilai_ta_skripsi') }}"><i class="fa fa-circle-o"></i> <span>
                        TA/Skripsi</span></a></li>
        </ul>
    </li>
    <li class="treeview">
        <a href="#">
            <i class="fa fa-database"></i> <span>BAP PraUSTA</span>
            <span class="pull-right-container">
                <i class="fa fa-angle-left pull-right"></i>
            </span>
        </a>
        <ul class="treeview-menu">
            <li><a href="{{ url('bap_pkl_magang') }}"><i class="fa fa-circle-o"></i> <span>
                        PKL/Magang</span></a></li>
            <li><a href="{{ url('bap_sempro') }}"><i class="fa fa-circle-o"></i> <span>
                        Seminar Proposal</span></a></li>
            <li><a href="{{ url('bap_ta_skripsi') }}"><i class="fa fa-circle-o"></i> <span>
                        TA/Skripsi</span></a></li>
        </ul>
    </li>
    <li class="treeview">
        <a href="#">
            <i class="fa fa-database"></i> <span>Export PraUSTA</span>
            <span class="pull-right-container">
                <i class="fa fa-angle-left pull-right"></i>
            </span>
        </a>
        <ul class="treeview-menu">
            <li><a href="{{ url('export_data') }}"><i class="fa fa-circle-o"></i> <span>
                        Export Data</span></a></li>
        </ul>
    </li>
    <li class="treeview">
        <a href="#">
            <i class="fa fa-database"></i> <span>Validasi PraUSTA</span>
            <span class="pull-right-container">
                <i class="fa fa-angle-left pull-right"></i>
            </span>
        </a>
        <ul class="treeview-menu">
            <li><a href="{{ url('validasi_prakerin') }}"><i class="fa fa-circle-o"></i> <span>
                        PKL</span></a></li>
            <li><a href="{{ url('validasi_sempro') }}"><i class="fa fa-circle-o"></i> <span>
                        Seminar Proposal</span></a></li>
            <li><a href="{{ url('validasi_ta') }}"><i class="fa fa-circle-o"></i> <span>
                        Tugas Akhir</span></a></li>
        </ul>
    </li>
    <li class="treeview">
        <a href="#">
            <i class="fa fa-database"></i> <span>Waktu PraUSTA</span>
            <span class="pull-right-container">
                <i class="fa fa-angle-left pull-right"></i>
            </span>
        </a>
        <ul class="treeview-menu">
            <li><a href="{{ url('waktu_pkl') }}"><i class="fa fa-circle-o"></i> <span>
                        PKL</span></a></li>
            <li><a href="{{ url('waktu_sempro') }}"><i class="fa fa-circle-o"></i> <span>
                        Seminar Proposal</span></a></li>
            <li><a href="{{ url('waktu_ta') }}"><i class="fa fa-circle-o"></i> <span>
                        Tugas Akhir</span></a></li>
        </ul>
    </li>
    <li class="treeview">
        <a href="#">
            <i class="fa fa-database"></i> <span>Honor PraUSTA</span>
            <span class="pull-right-container">
                <i class="fa fa-angle-left pull-right"></i>
            </span>
        </a>
        <ul class="treeview-menu">
            <li><a href="{{ url('honor_pkl') }}"><i class="fa fa-circle-o"></i> <span>
                        PKL</span></a></li>
            <li><a href="{{ url('honor_sempro') }}"><i class="fa fa-circle-o"></i> <span>
                        Seminar Proposal</span></a></li>
            <li><a href="{{ url('honor_ta') }}"><i class="fa fa-circle-o"></i> <span>
                        Tugas Akhir</span></a></li>
        </ul>
    </li>
</ul>
