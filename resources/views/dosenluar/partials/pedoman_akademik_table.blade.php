                ...
                <div class="box-body">
                    <div class="clearfix" style="margin-bottom: 12px;">
                        <span class="label bg-red pull-left" style="font-size: 12px; padding: 6px 10px;">
                            #{{ ($pedoman->firstItem() ?: 1) + $index }}
                        </span>
                        <span class="label label-success pull-right" style="font-size: 12px; padding: 6px 10px;">
                            {{ strtoupper(pathinfo($item->file, PATHINFO_EXTENSION) ?: 'FILE') }}
                        </span>
                    </div>

                    <div style="display: flex; align-items: flex-start;">
                        <div style="width: 52px; height: 52px; background: #fdf2f2; border-radius: 10px; text-align: center; line-height: 52px; margin-right: 15px;">
                        ...
                        <div style="flex: 1;">
                            ...
                            <p style="margin: 0 0 12px; color: #777; min-height: 40px;">
                                Dokumen pedoman akademik aktif untuk referensi dosen luar.
                            </p>
                        </div>
                    </div>

                    <div style="margin: 15px 0 12px; padding: 12px 14px; background: #f9fafc; border: 1px solid #eef1f5; border-radius: 8px;">
                        <div class="row">
                            <div class="col-xs-7">
                                <small style="display: block; color: #999; margin-bottom: 4px;">Tahun Akademik</small>
                                <span class="label label-primary" style="font-size: 12px;">{{ $item->periode_tahun }}</span>
                            </div>
                            <div class="col-xs-5 text-right">
                                <small style="display: block; color: #999; margin-bottom: 4px;">Jenis File</small>
                                <strong style="color: #444;">{{ strtoupper(pathinfo($item->file, PATHINFO_EXTENSION) ?: 'FILE') }}</strong>
                            </div>
                        </div>
                    </div>

                    <div class="clearfix">
                        <a href="{{ url('download_pedoman_dsn_luar/' . $item->id_pedomanakademik) }}"
...
<div class="row">
    <div class="col-sm-6">
        <div style="padding-top: 8px; color: #666;">
            Menampilkan {{ $pedoman->firstItem() ?: 0 }} sampai {{ $pedoman->lastItem() ?: 0 }} dari
            {{ $pedoman->total() }} pedoman
        </div>
    </div>
    <div class="col-sm-6 text-right">
        {{ $pedoman->appends(request()->only('q', 'per_page'))->links() }}
    </div>
</div>
